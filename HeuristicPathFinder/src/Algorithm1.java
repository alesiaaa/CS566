...
public class Algorithm1 {
	...
	/** 
	 * 	Algorithm 1: Among all 
	 *  nodes v that are 
	 * 	adjacent to the node n, 
	 * 	choose the one with
	 *  the smallest dd(v). 
	 */
	 ...
	 * Output: None.
	 * Use: Constructor for Algorithm1.
	 */
	public Algorithm1 (String node, HashMap <String, Integer> distance, 
	 ...
	 * shortest distance (from direct distance 
	 * file only). Ex. AB
	...
	private String getShortestSet (String node){
		...
		for (String s: adjacent){
			...
			if (distance.get(Character.toString(ch[1])) < miles 
					&& !this.bannedList.contains(Character.toString(ch[1]))
				){
				
				miles = distance.get(Character.toString(ch[1]));
				shortestSet = s;
